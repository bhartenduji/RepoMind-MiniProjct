...
CHECKPOINT_PATH = (
    "checkpoints/hybrid_transformer_bug_classifier.pt"
)

TOKENIZER_PATH = (
    "data/processed/code_tokenizer/tokenizer.json"
)
...
def get_numeric_features(record):
    ...
    added = record.get("added_line_count")

    if added is None:
        added = diff_stats.get("added_lines", 0)

    removed = record.get("removed_line_count")

    if removed is None:
        removed = diff_stats.get("deleted_lines", 0)
    ...
    actual_file_count = float(len(changed_files))
    ...
    for file_info in changed_files:
        ...
        if file_info.get("ignored", False):
            ignored_files += 1.0

    if files_changed <= 0:
        files_changed = actual_file_count

    denominator = max(files_changed, 1.0)

    avg_added = added / denominator
    avg_removed = removed / denominator
    avg_changed = total / denominator

    source_ratio = source_files / denominator
    test_ratio = test_files / denominator
...
def encode_patch(
    ...
    ids = tokenizer.encode(patch).ids
...
def main():
    ...
    print()
    print("=" * 50)
    print("INFERENCE / TEST-SPLIT VALIDATION")
    print("=" * 50)
